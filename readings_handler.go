...
func createReading(w http.ResponseWriter, r *http.Request) {
    ...
    sqlStatement := `INSERT INTO readings (userid, timestamp, value, torquevalues, motionwastes, setvalue, asmtimes) VALUES ($1, $2, $3, $4, $5, $6)`
    _, err = db.Exec(sqlStatement, reading.UserID, reading.Timestamp, reading.Value, pq.Array(reading.TorqueValues), pq.Array(reading.MotionWastes), reading.SetValue, pq.Array(reading.AsmTimes))
...
func getUserReadings(w http.ResponseWriter, r *http.Request) {
    ...
    rows, err := db.Query("SELECT id, userid, timestamp, value, torquevalues, motionwastes, setvalue, asmtimes FROM readings WHERE userid = $1", userID)
    if err != nil {
        http.Error(w, "Database query error", http.StatusInternalServerError)
        return
    }
